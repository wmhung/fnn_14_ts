      ...
      <ul className='flex gap-7 items-center'>
        ...
        <li>
          {!session?.user ? (
            ...
            // OAuth users → image from the OAuth provider (session token)
            <NaviDropdown name={session.user.name} image={session.user.image} />
          ) : (
            // Credentials (or any non-OAuth) users → avatar from the database
